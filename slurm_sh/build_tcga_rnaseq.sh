...
function to_db()
{
    ...
    aws --endpoint-url http://gdc-accessor1.osdc.io s3 cp s3://tcga_rnaseq_alignment_meta_3/${s3_url} .
...
function main()
{
    ...
    do
        ...
        aws --endpoint-url http://kh13-9.osdc.io --profile ceph s3 cp ${i::-1}_fastqc_db.db s3://tcga_rnaseq_fastq_db/${i}
        rm *.db *.log
    done
}
