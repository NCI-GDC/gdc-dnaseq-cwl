...
#### Description
The `bamfastq_align` workflow will accept BAM and/or FASTQ input, and align the reads to a user-supplied reference genome using bwa as part of its main transform. Each Read Group is aligned separately, and then merged into a single BAM, which then has duplicates marked with picard, and base scores recalibrated with GATK. The transform include a metrics subworkflow, which generates picard and samtools metrics on the aligned BAM. WGS metrics (picard CollectWgsMetrics) is implemented, and WXS (picard CollectHsMetrics) are collected when interval list files are supplied. Additional metrics collected include picard OxoGMetrics, picard CollectMultipleMetrics, and GATK CalculateContamination. All of the metrics are parsed into a single sqlite file.
