...
#set -e
cwltool --pack workflows/dnaseq/etl_http.cwl > etl_http_pack.cwl
docker_repos=($(grep dockerPull etl_http_pack.cwl | sort | uniq | awk '{print $2}' | sed -e 's/^"//' -e 's/"$//' | awk -F ":" '{print $1}' | grep ncigdc))
for docker_repo in "${docker_repos[@]}"
do
    docker pull ${docker_repo}:latest
done

rm etl_http_pack.cwl
